...
class ClusteringViewController : UIViewController, YMKClusterListener, YMKClusterTapListener {
    ...
    override func viewDidLoad() {
        ...
        self.mapView.mapWindow.map.move(with: cameraPosition)
        
        // Note that application must retain strong references to both
        // cluster listener and cluster tap listener
        let collection = self.map.mapObjects.addClusterizedPlacemarkCollection(with: self)!
    ...
    func clusterImage(_ clusterSize: UInt) -> UIImage {
        ...
        let size = text.size(attributes: [NSAttributedString.Key.font: font])
